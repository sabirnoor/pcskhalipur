...
//echo '<pre>';print_r($seat_booked_arr);die;
?>
<!DOCTYPE html>
...
<body>
				...
				<div class="row gutter">
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
						<div class="page-title">
							<h4>Bording - Dropoff Point - {{$array->fromcity_name}} <span class="icon-arrow-long-right"></span> {{$array->tocity_name}}</h4>
						</div>
					</div>
				...
				for($j=1;$j<=15;$j++){
					...
					if(isset($seat_lower_arr[$c+1][0]) && ($seat_lower_arr[$c][0]==$seat_lower_arr[$c+1][0])){
						...
						if(in_array($seat_lower_arr[$c][0],$seat_booked_arr)){
								$class=' class="availableST sleeper-horizontal-selected"';
						}else{
						...
						if(in_array($seat_lower_arr[$c][0],$seat_booked_arr)){
								$class=' class="availableST sleeper-vertical-selected"';
						}else{
						...
						if(in_array($seat_lower_arr[$c][0],$seat_booked_arr)){
								$class=' class="availableST "';
						}else{
								$class='class="availableST "';
						}
					}
					?>
		<td <?php if(isset($seat_lower_arr[$c][0]) && $seat_lower_arr[$c][0]<>""){ echo $class;echo $span;}?> id="<?=$seat_lower_arr[$c][0]?>" data='<?=$seat_lower_arr[$c][1]?>'><label for="<?php echo $seat_lower_arr[$c][0]; ?>"><input class="none checkbox" id="<?php echo $seat_lower_arr[$c][0]; ?>`" type="checkbox"><?php echo $seat_lower_arr[$c][0]; ?></label></td>
					<?php 
					$c++;
				}
				echo '</tr>';
	...
	<table border="0" cellspacing="0" cellpadding="0" class="no-margin">	
		...
		?>
								...
								<div class="col-md-6 create_table">
									<span id="viewFormSeaterTable"><table id="" width="100%" border="0" cellspacing="0" cellpadding="0" class="no-margin">
									<tr>
									<td>Seat Number</td>
									<td>Weekdays fare</td>
									<td>Weekend fare</td>
									...
									<tr>
										<td><div class="col-md-12" style="padding-right:2px; padding-left:0px;">
											<input type="text" readonly="readonly" name="seat_no[]" value="" id="seat_no" no-special-char ng-model="seat_no" maxlength="30" placeholder="seat no" class="form-control">
										</div></td>
										<td><div class="col-md-12" style="padding-right:2px; padding-left:0px;">
											<input type="text" name="rateWeekdays[]" maxlength="5" index="0" id="ratewd_0" class="form-control copypaste numericonly">
										</div></td>
										
										<td><div class="col-md-12" style="padding-right:2px; padding-left:0px;">
											<input type="text" name="rateWeekend[]" maxlength="5" index="0" id="ratewk_0" class="form-control copypaste numericonly">
										</div></td>
									</tr>
	...
	<script>
		...
		$('td.availableST input[type="checkbox"]').show();
		});
